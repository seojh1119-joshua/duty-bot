...
def load_excel(file_source) -> pd.DataFrame:
    """엑셀 파일(Path / BytesIO / bytes / UploadedFile) -> 정제된 DataFrame"""
    try:
        ...
        # 컬럼명 매핑 보완 (실제 근무1 -> 실제근무1 등)
        rename_dict = {
            "참고년월": "참고 년월",
            "실제근무_1": "실제근무1", "실제근무_2": "실제근무2",
            "근무자_1": "근무자1", "근무자_2": "근무자2",
            "대직_1": "대직1", "대직_2": "대직2"
        }
...
with tab_cal:
    ...
    # 🔍 상태 모니터링 디버그 패널
    with st.expander("🛠️ 데이터 상태 확인 (디버그)", expanded=False):
        st.write(f"- **총 데이터 수**: {len(st.session_state.master_df)}행")
        st.write(f"- **생성된 캘린더 이벤트 수**: {len(events)}개")
        st.write("- **실제근무1 입력 건수**: ", (st.session_state.master_df["실제근무1"] != "").sum())
        st.write("- **상위 3건 데이터 미리보기**:")
        st.dataframe(st.session_state.master_df[["날짜", "근무자1", "대직1", "실제근무1", "근무자2", "실제근무2"]].head(3))
    ...
    selected = calendar(events=events, options=calendar_options, key="duty_cal_v3")
