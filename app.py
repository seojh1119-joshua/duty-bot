...
def load_excel_smart(file_input, selected_sheet=None):
    ...
    # M열(13번째 열, 인덱스 12) 값 반영 (공백일 경우 빈 문자열 처리)
...
def settings_dialog():
    tab_s1, tab_s2, tab_s3 = st.tabs(["화면 설정", "순환근무 등록", "카카오톡 개인계정 연동"])
    ...
    with tab_s2:
        ...
        st.info("입력하신 근무자 주기에 맞춰 지정 기간 동안의 근무표를 자동으로 생성 및 엑셀에 반영합니다.")
        ...
        if st.button("🔄 순환근무 패턴 반영 및 저장", use_container_width=True, type="primary", key="dlg_apply_batch_btn"):
            ...
            st.success("✅ 순환근무 패턴이 반영 및 원본 엑셀에 저장되었습니다!")
            st.rerun()
...
def edit_worker_dialog(date_str, duty_info):
    ...
    with st.form(f"form_{date_str}"):
        ...
        sub1_s = st.selectbox("대직자1 (변경 시 선택)", worker_options, index=get_idx(curr_sub1), key=f"f_sub1_{date_str}")
        sub1_c = st.text_input("대직1 직접입력", value=curr_sub1 if sub1_s == "(직접 입력)" else "", key=f"f_sub1c_{date_str}") if sub1_s == "(직접 입력)" else ""

        st.markdown("---")
        st.markdown("##### 👥 근무자2 및 대직2 수정")
        ...
        sub2_s = st.selectbox("대직자2 (변경 시 선택)", worker_options, index=get_idx(curr_sub2), key=f"f_sub2_{date_str}")
        ...
        submitted = st.form_submit_button("💾 수정사항 저장 (엑셀 덮어쓰기)", use_container_width=True, type="primary")
    ...
    if submitted:
        ...
        # 원본 파일 내 근무자1, 대직1, 근무자2, 대직2 열 데이터 직접 수정 및 덮어쓰기 반영
        ...
        st.session_state.update({"editing_date": None, "editing_duty_info": None})
        st.success("✅ 근무자 수정 내용이 원본 엑셀 파일에 성공적으로 덮어쓰여졌습니다!")
        st.rerun()
...
with tab1:
    ...
    with col_c1:
        ...
        sel_month = st.selectbox("조회 월 선택", avail_months, index=avail_months.index(st.session_state.selected_month) if st.session_state.selected_month in avail_months else 0, key="calendar_month_select_tab1")
        st.session_state.selected_month = sel_month
    with col_c2:
        view_mode_choice = st.radio("달력 보기 방식", ["🗓️ 가로달력", "📄 세로달력"], index=0 if st.session_state.auto_view_type == "🗓️ 가로형 Grid" else 1, horizontal=True, key="calendar_view_mode_radio")
        if view_mode_choice == "🗓️ 가로달력":
    ...
    if sel_month in avail_months:
        ...
        for i, row in m_df.iterrows():
            ...
            # 📌 M열 값 반영 (공백인 경우 공백 유지, 값이 있으면 일자 앞에 [값] 형태로 표시)
...
with tab2:
    ...
    if st.button("변경사항 일괄 저장 (엑셀 반영)", use_container_width=True, type="primary", key="editor_save_btn"):
        ...
        st.success("✅ 에디터 변경사항이 원본 엑셀에 성공적으로 저장되었습니다.")
        st.rerun()
