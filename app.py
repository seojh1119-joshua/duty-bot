...
<style>
    ...
    /* 모바일 가로형 달력 자동 회전 및 가로 스크롤 보장 레이아웃 */
...
<script>
    ...
    function handleGesture() {
        ...
        if (touchendX < touchstartX - threshold) {
            // 왼쪽 스와이프 -> 다음달 버튼 클릭 유도
            const nextBtn = window.parent.document.querySelector('button[key="next_month_btn"], button[aria-label*="다음달"]');
            if (!nextBtn) {
                // 버튼 키나 aria-label로 못 찾을 경우 버튼 내용으로 검색
                const buttons = window.parent.document.querySelectorAll('button');
                buttons.forEach(btn => {
                    if (btn.innerText.includes('다음달')) {
                        btn.click();
                    }
                });
            } else {
        ...
        if (touchendX > touchstartX + threshold) {
            // 오른쪽 스와이프 -> 이전달 버튼 클릭 유도
            const buttons = window.parent.document.querySelectorAll('button');
            buttons.forEach(btn => {
                if (btn.innerText.includes('이전달')) {
                    btn.click();
                }
            });
        }
